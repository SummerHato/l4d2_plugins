...
public Action:SlotsRequest(client, args)
{
	...
	if (args == 1)
	{
		...
		else
		{
			if(client == 0)
			{
				CPrintToChatAll("{blue}[{default}Slots{blue}] {olive}admin (%s) {default}has limited Slots to {blue}%i {default}",sName, Int);
			...
			{
				CPrintToChat(client, "{blue}[{default}Slots{blue}] {default}You can't limit Slots lower than required Players.", MinSlots);
			}
