...
int
	...
	g_iFriendlyFireTaken[MAXPLAYERS+1],
	g_iTankDmg[MAXPLAYERS+1][MAXPLAYERS+1],
	g_iSortField;
	...
	if (IsValidSur(attacker) && IsPlayerAlive(attacker))
	{
		...
		else if (IsValidSur(victim) && IsPlayerAlive(victim))
		{
			if (attacker != victim)
			{
				...
				g_iFriendlyFireTaken[victim] += RoundToFloor(damage);
			}
		}
	}
	return Plugin_Continue;
...
void ShowTankDamageRank(int tank)
{
	...
	if (iCount > 0)
	{
		...
		if (!IsFakeClient(tank))
			CPrintToChatAll("{default}[{green}!{default}] {blue}Damage {default}dealt to {blue}Tank {default}({olive}%s{default}):", tank);
		else
			CPrintToChatAll("{default}[{green}!{default}] {blue}Damage {default}dealt to {blue}Tank {default}:");

		for (int i; i < iCount; i++)
			CPrintToChatAll("{blue}[{default}%i{blue}] ({default}%i%%{blue}) {olive}%N", data[i].damage, RoundToNearest(float(data[i].damage)/iTotalDmg*100), data[i].player);
	}
}
...
void ShowWitchDamageRank(int witch)
{
	...
	if (iCount > 0)
	{
		...
		CPrintToChatAll("{default}[{green}!{default}] {blue}Damage {default}dealt to {blue}Witch {default}:");

		for (int i; i < iCount; i++)
			CPrintToChatAll("{blue}[{default}%i{blue}] ({default}%i%%{blue}) {olive}%N", RoundToNearest(data[i].WitchDmg), RoundToNearest(data[i].WitchDmg/fTotalDmg*100), data[i].player);
	}
}
...
{
	killData[] data = new killData[MaxClients];
	int iCount, iTotalDmg, iTotalFF, iTotalFFTaken;
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i) && GetClientTeam(i) == 2)
		{
			...
			data[iCount].FFTaken = g_iFriendlyFireTaken[i];

			iTotalDmg += g_iTotalDmg[i];
			iTotalFF += g_iFriendlyFire[i];
			iTotalFFTaken += g_iFriendlyFireTaken[i];
			iCount++;
		}
	}
	...
	if (iCount > 0)
	{
		...
		CPrintToChatAll("{blue}[★MVP统计★]{default}:");

		for (int i; i < iCount; i++)
			CPrintToChatAll("{blue}伤害{default}: {yellow}%-6i {blue}特感{default}: {yellow}%-3i {blue}丧尸{default}: {yellow}%-4i {blue}友伤{default}: {yellow}%-5i {blue}被黑{default}: {yellow}%-5i {blue}| {olive}%N", 
				data[i].damage, data[i].SI, data[i].CI, data[i].FF, data[i].FFTaken, data[i].player);

		if (iTotalDmg > 0)
			CPrintToChatAll("{blue}[MVP]{default}: {olive}%N  {blue}伤害{default}: {yellow}%i {default}({yellow}%i{default}%%)", data[0].player, data[0].damage, RoundToNearest(float(data[0].damage)/iTotalDmg*100));
		
		if (iTotalFF > 0)
		{
			g_iSortField = killData::FF;
			SortCustom2D(data, iCount, SortByDescending);
			CPrintToChatAll("{blue}[持枪特感]{default}: {olive}%N  {blue}友伤{default}: {yellow}%i {default}({yellow}%i{default}%%)", data[0].player, data[0].FF, RoundToNearest(float(data[0].FF)/iTotalFF*100));
		}
		
		
		if (iTotalFFTaken > 0)
		{
			g_iSortField = killData::FFTaken;
			SortCustom2D(data, iCount, SortByDescending);
			CPrintToChatAll("{blue}[倒霉蛋]{default}: {olive}%N  {blue}被黑{default}: {yellow}%i {default}({yellow}%i{default}%%)", data[0].player, data[0].FFTaken, RoundToNearest(float(data[0].FFTaken)/iTotalFFTaken*100));
		}
	}
}
...
int Native_GetKillData(Handle plugin, int numParams)
{
	...
	data.FFTaken = g_iFriendlyFireTaken[player];
